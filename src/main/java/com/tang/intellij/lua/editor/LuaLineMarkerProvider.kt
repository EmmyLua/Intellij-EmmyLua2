...
class LuaLineMarkerProvider : LineMarkerProvider {
    ...
    private fun createNavigationHandler(
        ...
        return { mouseEvent, _ ->
            ...
            if (gutterInfo.kind == GutterKind.Override && gutterInfo.data is String) {
                val dataStr = gutterInfo.data as String
                    ...
                    if (languageServerItem != null) {
                        ...
                        if (server != null && gutterInfo.data != null) {
                            ...
                            if (response != null && response.locations.isNotEmpty()) {
                                // Convert locations to NavigatablePsiElements
                                val targets = mutableListOf<NavigatablePsiElement>()
                                
                                for (location in response.locations) {
                                    val virtualFile = VirtualFileManager.getInstance().findFileByUrl(location.uri)
                                    if (virtualFile != null) {
                                ...
                                if (targets.isNotEmpty()) {
                                    ApplicationManager.getApplication().invokeLater {
                                        if (targets.size == 1) {
                                            ...
                                            // Multiple targets, show popup using list selection
    ...
    private fun showNavigationPopup(
        ...
        // Create a list popup with custom renderer
        com.intellij.openapi.ui.popup.JBPopupFactory.getInstance()
            .createPopupChooserBuilder(targets)
            .setTitle(title)
            .setRenderer(com.intellij.ui.SimpleListCellRenderer.create { label, value, _ ->
                if (value is LuaGutterNavigatableElement) {
                    label.text = value.name
                    label.icon = value.presentation.getIcon(false)
                    
                    // Set gray text for location
                    val location = value.presentation.locationString
                    if (location != null) {
                        label.text = value.name
                        // Add location as grayed out suffix
                        label.foreground = com.intellij.ui.JBColor.foreground()
                    }
                } else {
                    label.text = value?.name ?: "Unknown"
                }
            })
            .setItemChosenCallback { selectedItem ->
                selectedItem.navigate(true)
            }
            .createPopup()
    ...
    /**
     * Create navigation handler for Override markers (legacy)
     */
...
class LuaGutterNavigatableElement(
    ...
    override fun navigate(requestFocus: Boolean) {
        ...
        val document = com.intellij.openapi.fileEditor.FileDocumentManager.getInstance()
            .getDocument(virtualFile) ?: return
        ...
        if (virtualFile != null) {
            ...
            if (document != null && line >= 0 && line < document.lineCount) {
                ...
                if (lineText.isNotEmpty()) {
                    ...
                    if (identifier != null) {
                        return "$identifier ($fileName:$lineNumber)"
                    }
                }
                ...
                return "$preview ($fileName:$lineNumber)"
            }
        }
...
/**
 * Custom renderer for gutter location list
 */
class LuaGutterLocationRenderer : DefaultPsiElementCellRenderer() {
    override fun getContainerText(element: PsiElement?, name: String?): String? {
        if (element is LuaGutterNavigatableElement) {
