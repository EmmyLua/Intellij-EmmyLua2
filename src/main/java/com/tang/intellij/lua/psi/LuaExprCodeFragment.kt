...
class LuaExprCodeFragment(project: Project, name: String, text: CharSequence, var myPhysical: Boolean) : LuaPsiFile(
    ...
    override fun clone(): LuaExprCodeFragment {
        val clone = cloneImpl(calcTreeElement().clone() as FileElement) as LuaExprCodeFragment
        copyCopyableDataTo(clone)
        clone.myPhysical = false
        clone.myOriginalFile = this
        val fileMgr = (manager as PsiManagerEx).fileManager
        val cloneViewProvider =
            fileMgr.createFileViewProvider(LightVirtualFile(name, language, text), false) as SingleRootFileViewProvider
